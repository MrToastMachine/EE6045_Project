...
%%%% sw is a switch that enables/disables plotting, it is useful for
%%%% parametric analisys when you do not want to plot all the points.
%%%% Comments: the function is fixed to use hann window.
%%%% Copyright UCC-Tyndall-MCCI, 2017.

function [snr, enob] = gs_fresp(x, N, fs, fi, OSR, sw)

%%%sw enables disables ploting options

%%%%Frequency response of a real signal%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%   Without normalization 
% outf=fft(x);
% outf_mag=abs(outf);     					  %FFT Single side.
% psd=10*log10(abs(outf_mag.^2));             %PowerSpectralDensity.

ventana=hann(N,'periodic');%rectwin(N);      %Window for the data.  
...
figure; plot([0:N/2-1].*(fs/N), psd(1:N/2)); %Plotting options
...
pot_noise_snr=10*log10(sum(abs(spectrum(3:bin_band)).^2)-sum(abs(spectrum(bin_signal-aux:bin_signal+aux).^2))-sum(abs(spectrum(2*bin_signal-aux-1:2*bin_signal+aux-1).^2))-sum(abs(spectrum(3*bin_signal-aux-2:3*bin_signal+aux-2).^2))-sum(abs(spectrum(4*bin_signal-aux-3:4*bin_signal+aux-3).^2))-sum(abs(spectrum(5*bin_signal-aux-4:5*bin_signal+aux-4).^2))); %%Noise power
%pot_noise_snr=10*log10(sum(abs(spectrum(1:bin_band)).^2)-sum(abs(spectrum(bin_signal-aux:bin_signal+aux).^2))-sum(abs(spectrum(2*bin_signal-aux-1:2*bin_signal+aux-1).^2))-sum(abs(spectrum(3*bin_signal-aux-2:3*bin_signal+aux-2).^2))); %%Noise power
%pot_noise_snr=10*log10(sum(abs(spectrum(3:bin_band)).^2)-sum(abs(spectrum(bin_signal-aux:bin_signal+aux).^2))-sum(abs(spectrum(2*bin_signal-aux-1:2*bin_signal+aux-1).^2))); %%Noise power

pot_noise_lab=10*log10(sum(abs(spectrum(3:bin_band)).^2));

sndr=pot_signal-pot_noise;                    %SNDR calculation
enob=(sndr-1.76)/6.02;                        %ENOB calculation   
snr=pot_signal-pot_noise_snr;                 %SNR calculation up-to 5th harmonic discarted 
...
set(gca,'FontSize',14)                       %Plotting options
grid on
legend(['SNR=' num2str(snr), '   ENOB=' num2str(enob)]);%Plotting options
end
